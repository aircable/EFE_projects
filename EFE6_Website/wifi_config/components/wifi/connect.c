...
void start_mdns_service(){
  ...
  ESP_LOGI( TAG, "MDNS name: %s", settings.ap_ssid);
  mdns_instance_name_set("UCSC_EFE");
}
...
deinit_wifi( void ){
    ...
    esp_wifi_disconnect();
    vTaskDelay( 200 / portTICK_PERIOD_MS) ; // needs time to stop
    esp_wifi_stop();
    vTaskDelay( 200 / portTICK_PERIOD_MS) ; // needs time to stop
    esp_wifi_deinit(); 
    // events are dublicated otherwise
    esp_event_handler_unregister( WIFI_EVENT, ESP_EVENT_ANY_ID, event_handler );
    esp_event_handler_unregister( IP_EVENT, ESP_EVENT_ANY_ID, event_handler );   

    //fprintf( stderr, "%s: Wifi stop\n", TAG );
}
...
esp_err_t wifi_connect_sta( void )
{
    ...
    strncpy((char *)wifi_config.sta.ssid, settings.ssid, sizeof(wifi_config.sta.ssid) - 1);
    strncpy((char *)wifi_config.sta.password, settings.password, sizeof(wifi_config.sta.password) - 1);
...
void wifi_connect_ap( void )
{
    ...
    wifi_config_t wifi_config;
    memset(&wifi_config, 0, sizeof(wifi_config_t));
    strncpy((char *)wifi_config.ap.ssid, settings.ap_ssid, sizeof(wifi_config.ap.ssid) - 1);
    strncpy((char *)wifi_config.ap.password, settings.password, sizeof(wifi_config.ap.password) - 1);
